      ...
      <div className="w-full max-w-md">
        <div className="mb-8 flex flex-col items-center gap-3 text-center">
          {logoUrl ? (
            ...
            <img src={logoUrl} alt={merchantName} className="h-16 w-16 rounded-2xl object-cover shadow-soft" />
          ) : (
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl font-display text-2xl font-semibold text-white shadow-soft" style={{ backgroundColor: primaryColor }}>
              {merchantName.charAt(0).toUpperCase()}
            </div>
          )}
          <p className="font-sans text-sm font-medium uppercase tracking-[0.18em] text-slate-450">{merchantName}</p>
        </div>

        <div className="card-surface overflow-hidden px-6 py-10 sm:px-10 sm:py-12">
          <AnimatePresence mode="wait">
            {step === "rating" && (
              <motion.div key="rating" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }} className="flex flex-col items-center gap-8 text-center">
                <h1 className="font-display text-[26px] font-medium leading-tight text-ink-900 sm:text-3xl">Comment s&apos;est passée<br /> votre expérience&nbsp;?</h1>
                <StarRating value={rating} onChange={handleSelectRating} accentColor={accentColor} />
              </motion.div>
            )}

            {step === "sending" && (
              <motion.div key="sending" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center gap-4 py-10 text-center">
                <div className="h-8 w-8 animate-spin rounded-full border-2 border-ink-800/10" style={{ borderTopColor: accentColor }} />
                <p className="font-sans text-sm text-slate-450">Un instant…</p>
              </motion.div>
            )}

            {step === "thanks-positive" && (
              <motion.div key="thanks-positive" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="relative flex flex-col items-center gap-6 text-center">
                <Confetti accentColor={accentColor} />
                <motion.div initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 260, damping: 16 }} className="flex h-16 w-16 items-center justify-center rounded-full" style={{ backgroundColor: `${accentColor}22` }}>
                  <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={accentColor} strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M20 6L9 17l-5-5" /></svg>
                </motion.div>
            ...
            {step === "thanks-negative" && (
              <motion.div key="thanks-negative" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex flex-col items-center gap-5 py-4 text-center">
                <div className="flex h-16 w-16 items-center justify-center rounded-full" style={{ backgroundColor: `${accentColor}22` }}>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={accentColor} strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 21s-7.5-4.6-9.6-9.1C.9 8.3 2.6 5 6 5c2 0 3.4 1.1 4 2 .6-.9 2-2 4-2 3.4 0 5.1 3.3 3.6 6.9C19.5 16.4 12 21 12 21z" /></svg>
                </div>
...
function FeedbackForm({
  ...
  return (
    <motion.form initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }} onSubmit={handleSubmit} className="flex flex-col gap-5 text-left">
      <div className="text-center"><h2 className="font-display text-2xl font-medium text-ink-900">Aidez-nous à progresser</h2><p className="mt-1 font-sans text-sm text-slate-450">Ce message reste privé, il n&apos;est jamais publié en ligne.</p></div>
...
function Confetti({ accentColor }: { accentColor: string }) {
  ...
  return <div className="pointer-events-none absolute inset-x-0 top-0 flex justify-center overflow-hidden">{pieces.map((_, i) => <span key={i} className="absolute top-0 block h-2 w-2 animate-confetti rounded-sm" style={{ left: `${(i / pieces.length) * 100}%`, backgroundColor: i % 2 === 0 ? accentColor : "#14141c", animationDelay: `${i * 0.05}s` }} />)}</div>;
}
